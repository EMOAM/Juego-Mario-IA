...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    fill(255, 255, 255);
    textSize(40);
    textAlign(CENTER);//cambiar presiona el boton jugar
    text("Presiona el botón jugar", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function manualControl(character){
  ...
  if(character.live){
    ...
    //se regresa a control.right
    if(!keyDown(control.left)&&!keyDown(control.right)&&!keyDown(control.up)){ 
      character.changeAnimation('stand');//se quita codigo
    }
  }
